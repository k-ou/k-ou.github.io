! function (a, b) {
    ...
    const a = !("onscroll" in window) || /glebot/.test(navigator.userAgent),
        b = function (a) {
            return a.getBoundingClientRect().top + window.pageYOffset - a.ownerDocument.documentElement.clientTop
        },
        c = function (a, c, d) {
            return (c === window ? window.innerHeight + window.pageYOffset : b(c) + c.offsetHeight) <= b(a) - d
        },
        d = function (a) {
        ...
        e = function (a, b, c) {
            ...
            return (b === window ? e + window.pageXOffset : d(b) + e) <= d(a) - c
        },
        f = function (a, c, d) {
    ...
    class k {
        ...
        _setSourcesForPicture(a, b) {
            const c = a.parentElement;
            if ("PICTURE" === c.tagName)
                for (let a = 0; a < c.children.length; a++) {
                    let d = c.children[a];
                    if ("SOURCE" === d.tagName) {
                        let a = d.getAttribute("data-" + b);
                        a && d.setAttribute("srcset", a)
                    }
                }
        }
        _setSources(a, b, c) {
            ...
            if ("IMG" === d) {
                this._setSourcesForPicture(a, b);
                const c = a.getAttribute("data-" + b);
                return c && a.setAttribute("srcset", c), void(e && a.setAttribute("src", e))
            }
        ...
        _loopThroughElements() {
            ...
            for (e = 0; e < d; e++) {
                let d = c[e];
                b.skip_invisible && null === d.offsetParent || (a || h(d, b.container, b.threshold)) && (this._showOnAppear(d), f.push(e), d.wasProcessed = !0)
            }
            for (; f.length > 0;) c.splice(f.pop(), 1), i(b.callback_processed, c.length);
        ...
        handleScroll() {
            const a = this._settings.throttle;
            if (0 !== a) {
                const b = () => {
                    (new Date).getTime()
                };
                let c = b(),
                    d = a - (c - this._previousLoopTime);
